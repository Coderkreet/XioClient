...
const AdminPrivacyPolicy = () => {
  ...
  const fetchPolicy = async () => {
    setLoading(true);
    try {
      const res = await getAllPrivacyPolicies();
      let doc = null;
      if (res && res.data) {
      ...
      if (doc && Array.isArray(doc.privacyPolicy)) {
  ...
  const handleAddSection = () => {
    setPrivacyPolicy((prev) => [...prev, { title: "", description: "" }]);
  };
  ...
  const handleRemoveSection = (idx) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setPrivacyPolicy((prev) => prev.filter((_, i) => i !== idx));
  ...
  const handleSave = async (e) => {
    ...
    if (
      ...
      privacyPolicy.some((item) => !item.title || !item.description)
    ) {
    ...
    try {
      ...
      let res;
      if (docId) {
        res = await updatePrivacyPolicy(docId, payload);
        Swal.fire("Updated", "Privacy policy updated successfully!", "success");
      } else {
        res = await createPrivacyPolicy(payload);
        Swal.fire("Created", "Privacy policy created successfully!", "success");
      }
      if (res?.data?._id) setDocId(res.data._id);
      await fetchPolicy();
    } catch {
      Swal.fire("Error", "Failed to save privacy policy.", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="rounded-lg p-8 mb-8 mt-8" style={{ backgroundColor: "#120540", borderColor: "#433C73", borderWidth: "1px" }}>
      <div className="border-b pb-6 mb-8" style={{ borderColor: "#433C73" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-semibold" style={{ color: "#AEA7D9" }}>
              Privacy Policy Management
            </h2>
            <p className="mt-1" style={{ color: "#727FA6" }}>
              Add, edit, and remove sections for your Privacy Policy
            </p>
          </div>
        </div>
      </div>
