...
const AdminCookiePolicy = () => {
  ...
  const fetchPolicy = async () => {
    ...
    try {
      ...
      if (doc && Array.isArray(doc.cookiePolicy)) {
        setCookiePolicy(doc.cookiePolicy);
      ...
      Swal.fire("Error", "Failed to fetch cookie policy", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddSection = () => {
    setCookiePolicy((prev) => [...prev, { title: "", description: "" }]);
  };
  ...
  const handleRemoveSection = (idx) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setCookiePolicy((prev) => prev.filter((_, i) => i !== idx));
  ...
  const handleSave = async (e) => {
    ...
    if (
      ...
      cookiePolicy.some((item) => !item.title || !item.description)
    ) {
    ...
    setLoading(true);
    try {
      const payload = { cookiePolicy };
      let res;
      if (docId) {
        res = await updateCookiePolicy(docId, payload);
        Swal.fire("Updated", "Cookie policy updated successfully!", "success");
      } else {
        res = await createCookiePolicy(payload);
        Swal.fire("Created", "Cookie policy created successfully!", "success");
      }
    ...
    <div className="rounded-lg p-8 mb-8 mt-8" style={{ backgroundColor: "#120540", borderColor: "#433C73", borderWidth: "1px" }}>
      <div className="border-b pb-6 mb-8" style={{ borderColor: "#433C73" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-semibold" style={{ color: "#AEA7D9" }}>Cookie Policy Management</h2>
            <p className="mt-1" style={{ color: "#727FA6" }}>
              Add, edit, and remove sections for your Cookie Policy
            </p>
          </div>
        </div>
      </div>
