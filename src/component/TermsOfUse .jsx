...
import { FileText, Shield, AlertCircle, CheckCircle, Lock, Globe, Scale, Users, Clock, Eye } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { getAllTermsAndConditions } from '../api/admin-api';
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#120540] via-[#433C73] to-[#4A088C] text-white overflow-hidden relative">
      ...
      <div className="relative z-20 pt-8 px-6 max-w-7xl mx-auto">
        <nav className="flex items-center text-[#AEA7D9] text-base font-medium space-x-2">
          ...
          <span className="text-[#727FA6]">Terms of Use</span>
        </nav>
      </div>
        ...
        <div className="grid lg:grid-cols-1 gap-8">
          ...
          <div className={`lg:col-span-3 transform transition-all duration-1000 delay-500 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
            <div className="bg-[#433C73]/70 p-8 rounded-3xl border border-[#727FA6] shadow-2xl">
              {loading ? (
                <div className="text-center text-[#AEA7D9] py-8">Loading Terms Of Use...</div>
              ) : error ? (
                <div className="text-center text-red-400 py-8">{error}</div>
              ) : sections.length === 0 ? (
                <div className="text-center text-[#AEA7D9] py-8">No Terms Of Use found.</div>
              ) : (
      ...
      <div className="relative z-10 bg-[#120540] border-t border-[#433C73] py-12">
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-8 text-center">
          <div>
            ...
            <p className="text-[#727FA6]">Fully compliant with international regulations</p>
          </div>
          <div>
            <h3 className="text-xl font-bold text-white mb-4">Regular Updates</h3>
            <p className="text-[#727FA6]">Terms updated to reflect current practices</p>
          </div>
          <div>
            <h3 className="text-xl font-bold text-white mb-4">User Protection</h3>
            <p className="text-[#727FA6]">Designed to protect both users and platform</p>
          </div>
        </div>
      </div>
      <Footer />
